  ...
  "action": {
    ...
    "page": {
      "load": function (e) {
        ...
        const url = e && e.top ? e.top : '';
        const isExcluded = config.isUrlExcluded(url);
        
        // If URL is excluded, send disabled state regardless of global state
        const effectiveState = isExcluded ? "disabled" : config.addon.state;
    ...
    "options": {
      ...
      "exclusions": function (e) {
        config.addon.exclusions = e.exclusions;
        /*  */
        // Update all tabs to check new exclusion rules
        app.page.send("storage", {
      ...
      "load": function () {
        app.options.send("storage", {
          ...
          "devices": config.addon.devices,
          "additional": config.addon.additional,
          "exclusions": config.addon.exclusions
        });
      }
    }
  }
...
app.options.receive("exclusions", core.action.options.exclusions);
